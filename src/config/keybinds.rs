...
pub struct NormalKeybindsConfig {
    ...
    pub create_clip: Keybind,
    pub recent_channels_search: Keybind,
    pub followed_channels_search: Keybind,
    pub live_channels: Keybind,
    pub chat_stats: Keybind,
    pub audio_toggle: Keybind,
    pub tts_toggle: Keybind,
    pub stream_viewer: Keybind,
    pub tab_new: Keybind,
    pub tab_next: Keybind,
    pub tab_prev: Keybind,
    pub tab_close: Keybind,
    pub help: Keybind,
    pub quit: Keybind,
}
...
impl Default for NormalKeybindsConfig {
    fn default() -> Self {
        Self {
            ...
            create_clip: Box::new([Key::Char('C')]),
            recent_channels_search: Box::new([Key::Char('s')]),
            followed_channels_search: Box::new([Key::Char('f')]),
            live_channels: Box::new([Key::Char('l')]),
            chat_stats: Box::new([Key::Char('m')]),
            audio_toggle: Box::new([Key::Char('a')]),
            tts_toggle: Box::new([Key::Char('T')]),
            stream_viewer: Box::new([Key::Char('V')]),
            tab_new: Box::new([Key::Char('t')]),
            tab_next: Box::new([Key::Tab]),
            tab_prev: Box::new([Key::BackTab]),
            tab_close: Box::new([Key::Ctrl('w')]),
            help: Box::new([Key::Char('?'), Key::Char('h')]),
            quit: Box::new([Key::Char('q')]),
        }
    }
